...
For `qwen-code-setup`, start by reading:
...
- do not plan as if the Qwen Code installer runtime already exists unless the task is
  specifically about building it
- preserve product and security invariants from `AGENTS.md`, especially around
  Qwen Code config targets, secret handling, and truthful scaffold status
...
- [ ] The plan stays truthful to current `qwen-code-setup` reality
