...
For `qwen-code-setup`, simplify in a way that preserves repository truth:

- read `AGENTS.md` before making contract-adjacent simplifications
- preserve the current scaffold reality that the Qwen Code installer runtime is not yet implemented unless the task explicitly changes that
- do not simplify away security constraints around secret handling, Qwen Code settings, config precedence, or truthful diagnostics
