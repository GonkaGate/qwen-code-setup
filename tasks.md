...
- [ ] CI matrix passes on supported operating systems.
      Blocked in this local session: the current implementation tree has no
      commit (`git rev-parse --verify HEAD` fails), all files are untracked,
      and `gh` is not authenticated, so GitHub Actions cannot be triggered or
      observed for this exact tree.
...
- [ ] Tasks 10.1 through 10.4 are complete.
      Blocked only by remote CI matrix pass evidence; local implementation,
      tests, docs, and package proof are complete.
